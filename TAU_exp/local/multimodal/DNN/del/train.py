...
def training(config):
    ...
    elif config['modal'] == 'representation':
    ...
    train_examples_len = len(dataset.dev_dataset)
    ...
    with open(os.path.join(resultsdir,
                           'besttrainacc_' + str(besttrainacc)[:6] + '_besttestacc_' + str(testacc)[:6] + ".json"), 'w',
              encoding='utf-8') as f:
        json.dump(outpre, f, ensure_ascii=False, indent=4)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(datasetdir, "Development.json"), encoding="utf8") as json_file:
        traindict = json.load(json_file)
    with open(os.path.join(datasetdir, "Evaluation.json"), encoding="utf8") as json_file:
    ...
    for workdict, dset in zip([traindict, devdict], ["train", "dev"]):
